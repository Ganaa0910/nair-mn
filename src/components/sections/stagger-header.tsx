...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const isCurrentSectionLight = isLightColor(currentSectionColor);
  const adaptiveTextColor = isCurrentSectionLight ? "#472913" : "#fff7ed";
  const adaptiveMenuButtonColor = isCurrentSectionLight
    ? "#472913"
    : menuButtonColor || "#fff7ed";
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (toggleBtnRef.current)
        gsap.set(toggleBtnRef.current, { color: adaptiveMenuButtonColor });
    });
    return () => ctx.revert();
  }, [adaptiveMenuButtonColor, position]);
  ...
  const animateColor = useCallback(
    (opening: boolean) => {
      ...
      if (changeMenuColorOnOpen) {
        ...
        gsap.set(btn, { color: adaptiveMenuButtonColor });
      }
    },
    [openMenuButtonColor, adaptiveMenuButtonColor, changeMenuColorOnOpen],
  );
  ...
  }, [changeMenuColorOnOpen, adaptiveMenuButtonColor, openMenuButtonColor]);
        ...
        >
          ...
          >
            <img
              ...
              alt="Logo"
              className="sm-logo-img block h-8 w-auto object-contain transition-opacity hover:opacity-80"
              draggable={false}
              width={110}
              height={24}
            />
            {logoText && (
              <span
                className="sm-logo-text font-bold text-[16px] md:text-[18px] tracking-[-0.3px] transition-all duration-300 hover:opacity-80"
