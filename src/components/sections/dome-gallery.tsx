...
type ImageItem = string | { src: string; alt?: string; videoId?: string };
...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  const normalizedImages = pool.map((image) => {
    if (typeof image === "string") {
      return { src: image, alt: "", videoId: undefined };
    }
    return { src: image.src || "", alt: image.alt || "", videoId: image.videoId };
  });
...
export default function DomeGallery({
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    const videoId = parentElement.dataset.videoId;
    
    // If it's a YouTube video, redirect to YouTube
    if (videoId) {
